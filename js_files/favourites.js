/* selecting the container from DOM where cards can be stored*/

let cardContainer=document.getElementById('container');


/* when page is loaded event-listener attached to DOM is executed*/
window.addEventListener("load",function(){

    /*getting the favourites array from local storage*/
    let favourites=this.localStorage.getItem("favouriteCharacters")
    
    //if favourites is null then we display nothing and return back
    if(favourites == null){
        cardContainer.innerHTML="<p class=\"no-character\">No characters are present in Favourites</p>"
        return;
    }
    //if characters are present ,then parsing them to convert it to array
    else{
        favourites=JSON.parse(this.localStorage.getItem("favouriteCharacters"));

    }
         // if all the characters are removed from favourites and no characters are left for displaying
