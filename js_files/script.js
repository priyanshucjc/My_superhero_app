...
//https://gateway.marvel.com:443/v1/public/characters?ts=<time-stamp>&apikey=545062c65900955cd41f074aba0ca4d2&hash=<md5(ts+privateKey+publicKey)


//*-------------------------------------- Selecting the element from DOM ----------------------------------------------------
let PUBLIC_KEY = "545062c65900955cd41f074aba0ca4d2";
let PRIVATE_KEY = "48949903ba1e2db9c3409bd23eb8aaae7d64dc1c";
let ts = 1;
let hash="08e143dc7704a55ed22b324bc51e6b3b";
...
async function searchHeros(textSearched) {
     ...
     // await fetch(`https://gateway.marvel.com/v1/public/characters?nameStartsWith=${textSearched}&apikey=9ab871748d83ae2eb5527ffd69e034de&hash=${hash}?ts=${ts}`)
     ...
     let res = await fetch(`https://gateway.marvel.com/v1/public/characters?nameStartsWith=${textSearched}&apikey=${PUBLIC_KEY}&hash=${hash}&ts=${ts}`);
     let data = await res.json() //Converting the data into JSON format
...
function showSearchedResults(searchedHero) {
     ...
     if(favouritesCharacterIDs == null){
          // If we didn't get the IDs of the favourite characters then we iniitalize it with empty map
